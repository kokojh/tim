...
function gotBuffers(buffers) {
	...
	// the ONLY time gotBuffers is called is right after a new recording is
	// completed -
...
function doneEncoding(blob) {
	Recorder.setupDownload(blob, "timaudio" + ((recIndex < 10) ? "0" : "")
			+ recIndex + ".wav");
	recIndex++;
}
...
//		audio.play();
		...
		var style = window.getComputedStyle(downloadbtn);
		var opacityValue = style.getPropertyValue("opacity");
// 		alert(opacityValue);
		if(opacityValue!=0.25){
			// start recording
			if (confirm("새로 녹음을 시작합니다. 이전에 녹음한 " +
					"파일을 다운로드 받지 않았다면 내용을 잃게 됩니다. " +
					"계속하시겠습니까?")){
				if (!audioRecorder)
					return;
